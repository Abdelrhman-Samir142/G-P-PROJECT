    ...
    useEffect(() => {
        if (isEditMode) {
            const fetchProduct = async () => {
                try {
                    const product = await productsAPI.get(editId);
                    setFormData({
                        title: product.title,
                        ...
                        auction_start_time: product.auction_start_time ? product.auction_start_time.slice(0, 16) : '',
                        auction_end_time: product.auction_end_time ? product.auction_end_time.slice(0, 16) : '',
                    });

                    if (product.images && product.images.length > 0) {
                        setImagePreviews(product.images.map((img: any) => img.image));
                    }
                } catch (err) {
